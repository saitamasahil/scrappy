...
if has_lpeg then
    ...
    local utf8_char = lpeg.R('\x20\x7f') + lpeg.R('\xc0\xdf') * cont + lpeg.R('\xe0\xef') * cont * cont +
...
local function cache_apperance(obj, cache, option)
    if not cache.visited_tables then
        cache.visited_tables = setmetatable({}, {
            __mode = 'k'
        })
    end
    local t = type(obj)
...
function pprint.pformat(obj, option, printer)
    ...
    local function table_formatter(t)
        ...
        local function print_kv(k, v, t)
            ...
            if formatter[type(v)] == nop_formatter or formatter[type(k)] == nop_formatter or
    ...
    formatter['function'] = option.show_function and make_fixed_formatter('function', option.object_cache) or
                                nop_formatter
    formatter['thread'] = option.show_thread and make_fixed_formatter('thread', option.object_cache) or nop_formatter
    formatter['userdata'] = option.show_userdata and make_fixed_formatter('userdata', option.object_cache) or
                                nop_formatter
