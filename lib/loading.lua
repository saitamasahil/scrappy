...
local highlight_position = {
    x = -logo:getWidth(),
    y = -logo:getHeight()
} -- Start above and to the left of the logo

function loading.new(type, update_duration)
    return setmetatable({
        type = type or "spinner",
        update_duration = update_duration or 0.5
    }, loading)
end

function loading:load()
    spin = function()
        timer.tween(self.update_duration, rotation, {
            value = rotation.value + 1
        }, 'linear', spin)
    end
    flash_start = function()
        timer.tween(self.update_duration * 0.5, opacity, {
            value = 0.3
        }, 'in-out-quad', flash_end)
    end
    flash_end = function()
        timer.tween(self.update_duration * 0.5, opacity, {
            value = 1
        }, 'in-out-quad', flash_start)
    end
    highlight = function()
        timer.tween(self.update_duration, highlight_position, {
            x = logo:getWidth(),
            y = logo:getHeight()
        }, 'linear', function()
