...
function splash.load(delay)
    delay = delay or 1
    timer.tween(delay, anim, {
        value = 1
    }, 'in-out-cubic')
    timer.after(delay + 0.2, function()
        timer.tween(0.5, anim, {
            value = 0
...
function splash.draw()
    ...
    love.graphics.draw(logo, 0, -anim.value * half_logo_height, 0, logo_scale, logo_scale, logo:getWidth() * 0.5,
        half_logo_height)
    love.graphics.setColor(colors.main[1], colors.main[2], colors.main[3], anim.value)
    love.graphics.push()
    love.graphics.translate(0, half_logo_height)
    love.graphics.draw(app_name, -app_name:getWidth() * 0.5, -anim.value * app_name:getHeight())
    love.graphics.pop()
    love.graphics.push()
