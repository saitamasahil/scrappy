...
ASSET_URL=$(echo "$RELEASE_DATA" | grep -oP '"browser_download_url": "\K[^"]+Scrappy_'${TAG}'_update\.muxzip')
if [ -z "$ASSET_URL" ]; then
    # If "Scrappy_{tag}_update.muxapp" is not found, look for any "Scrappy_{tag}*.zip"
    ASSET_URL=$(echo "$RELEASE_DATA" | grep -oP '"browser_download_url": "\K[^"]+Scrappy_'${TAG}'[^"]*\.muxzip')
    if [ -z "$ASSET_URL" ]; then
