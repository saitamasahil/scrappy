...
local user_config = setmetatable({}, {
    __index = config
})
...
function user_config:load_platforms()
    ...
    for _, item in ipairs(platforms) do
                ...
                if file then
                    ...
                    if not err and folder_name then
                        ...
                        if not result then
                            ...
                            if fn:find("dosbox") then
                                result = "pc"
                            end
                            if fn:find("msx") or fn:find("bluemsx") then
                                result = "msx"
                            end
                        end
                        return result
        ...
        if not assignment and parent then
            ...
            if assignment then
                log.write(string.format("Inherited platform '%s' for '%s' from parent folder '%s'", assignment, item,
                    parent))
            end
        end
        ...
        if assignment then
            ...
            if assignment == "ngpc" then
                ...
                for _, f in ipairs(files) do
                    local fl = f:lower()
                    if fl:match("%.ngp$") then
                        has_ngp = true
                    end
                    if fl:match("%.ngc$") or fl:match("%.ngpc$") then
                        has_ngpc_like = true
                    end
                    if has_ngpc_like then
                        break
                    end
                end
...
setmetatable(skyscraper_config, {
    __index = config
})
